...
def read_csv(file_path: str, delimiter: str) -> List[Tuple[str]]:
    """
    Read csv file
    """
    with open(file_path, 'r') as f:
        data = [tuple(x.strip().split(delimiter)) for x in f.readlines()]
    return data
...
def filter_NCore(data: List[Tuple[int, int, int]], core: int) -> List[Tuple[int, int]]:
    """
...
def process_movielens(name: str, file_path: str) -> None:
    """
    Process Movielens dataset, return processed interactions
    """
    # read data
    ratings = read_csv(file_path, delimiter='::')
    print(f'Number of ratings: {len(ratings)}')
    # count the distribution of stars
    star_cnt = Counter([int(x[2]) for x in ratings])
    print(f'Star distribution: {star_cnt}')
    # drop the duplicated ratings, filter < 3 stars
    ratings = list(set([(x[0], x[1], int(x[3])) for x in ratings if float(x[2]) >= 3]))
...
if __name__ == '__main__':
    process_movielens('Movielens-1M', 'raw/ratings.dat')
