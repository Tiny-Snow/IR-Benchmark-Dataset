...
def read_csv(file_path: str, skip_header: bool = False) -> List[Tuple[str]]:
    """
    Read csv file
    """
...
def process_food(name: str, train_file_path: str, valid_file_path: str, test_file_path: str) -> None:
    """
    Process Food dataset, return processed interactions
    """
    # read data
    train_data = read_csv(train_file_path, skip_header=True)
    valid_data = read_csv(valid_file_path, skip_header=True)
    test_data = read_csv(test_file_path, skip_header=True)
    reviews = train_data + valid_data + test_data   # merge all reviews
    print(f'Number of reviews: {len(reviews)}')
    # count the distribution of stars
    star_cnt = Counter([float(x[3]) for x in reviews])
    print(f'Star distribution: {star_cnt}')
    # drop the duplicated reviews (very few), filter < 3 stars
    # timestamp e.g. 2005-12-23
    reviews = list(set([(x[0], x[1], int(time.mktime(time.strptime(x[2], '%Y-%m-%d'))))
        for x in reviews if float(x[3]) >= 3]))
...
if __name__ == '__main__':
    process_food('Food', './raw/interactions_train.csv', './raw/interactions_validation.csv', './raw/interactions_test.csv')
