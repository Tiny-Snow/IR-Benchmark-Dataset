...
def read_csv(file_path: str, delimiter: str = ',', skip_header: bool = False) -> List[Tuple[str]]:
    """
    Read csv file
    """
...
def write_dataset(data: List[Tuple[int, int]], name: str, core: int, folder: str, train_ratio: float = 0.8) -> None:
    ...
    NOTE: We use popularity-shift OOD setting.
    ...
    # construct uniform test set
    train_data, test_data = [], []
    test_iterations = len(data) - int(train_ratio * len(data))
    while len(test_data) < test_iterations:
        i = random.choice(list(item_users.keys()))
        u = random.choice(item_users[i])
        if len(item_users[i]) > 1:  # make sure the train set has item i
            test_data.append((u, i))
            item_users[i].remove(u)
    # construct train set by removing test interactions
    train_data = [(u, i) for i in item_users for u in item_users[i]]
    # delete all users that only appear in test set
    train_users = set([x[0] for x in train_data])
    test_data = [(u, i) for u, i in test_data if u in train_users]
...
def process_douban(name: str, file_path: str) -> None:
    """
    Process Douban dataset, return processed interactions
    """
    # read data
    ratings = read_csv(file_path, delimiter='\t', skip_header=True)
    print(f'Number of ratings: {len(ratings)}')
    # count the distribution of stars
    star_cnt = Counter([float(x[2]) for x in ratings])
    print(f'Star distribution: {star_cnt}')
    # drop the duplicated ratings, filter < 3 stars
    ratings = list(set([(x[0], x[1]) for x in ratings if float(x[2]) >= 3]))
    print(f'Number of ratings without duplicates and >= 3 stars: {len(ratings)}')
    print(f'Number of users: {len(set([x[0] for x in ratings]))}')
    print(f'Number of items: {len(set([x[1] for x in ratings]))}')
    # filter by N-core
    core = 10
    proc_data = filter_NCore(ratings, core)
    print(f'Number of ratings after {core} core filtering: {len(proc_data)}')
    ...
    # write dataset
    write_dataset(proc_data, name, core, './proc', 0.8)


if __name__ == '__main__':
    process_douban('Douban-Movie', './raw/douban_movie.tsv')
