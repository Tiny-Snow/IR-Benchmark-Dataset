...
def process_yelp(year: int) -> None:
    """
    Process Yelp dataset
    """
    # count the amount of data
    reviews = read_json(f'raw/yelp_academic_dataset_review.json')
    print(f'Number of reviews: {len(reviews)}')
    # count the distribution of stars
    star_cnt = Counter([x['stars'] for x in reviews])
    print(f'Star distribution: {star_cnt}')
    # drop the duplicated reviews (very few), filter < 3 stars
    reviews = list(set([(x['user_id'], x['business_id']) for x in reviews if x['stars'] >= 3]))
    ...
    # write dataset
    write_dataset(proc_data, f'Yelp{year}', core, './proc', 0.8)
    

if __name__ == '__main__':
    process_yelp(year = 2018)
